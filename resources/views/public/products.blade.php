<x-public-layout :settings="$settings" title="Produk Platinum Gym Padang" description="Katalog produk makanan, minuman, suplemen, dan perlengkapan fitness di Platinum Gym Padang.">
    @include('public.partials.page-hero', [
        ...
        'primaryUrl' => '#produk-katalog',
        'primaryLabel' => 'Lihat Katalog',
        'secondaryUrl' => route('public.location'),
        'secondaryLabel' => 'Beli di Lokasi',
    ])

    <section id="produk-katalog" class="public-section public-section-muted scroll-mt-24">
        <div class="public-container">
            <div class="public-product-notice public-motion-reveal mb-8 grid gap-5 pr-20 sm:pr-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center lg:pr-6" data-motion="reveal">
                <div>
                    <p class="text-xs type-control uppercase tracking-[0.12em] text-zinc-600 dark:text-gold-400">Informasi Pembelian</p>
                    <p class="mt-2 max-w-3xl break-words text-sm type-control leading-7 text-zinc-700 dark:text-zinc-200">Pembelian produk dilakukan langsung di lokasi Platinum Gym Padang. Stok diperbarui oleh admin sesuai ketersediaan di lokasi.</p>
            ...
            <form method="GET" action="{{ route('public.products') }}" class="public-product-filter public-motion-reveal grid gap-4 pr-20 sm:pr-5 md:p-7 lg:grid-cols-[0.85fr_1fr_auto] lg:items-end" aria-describedby="products-filter-status" data-motion="reveal" data-motion-delay="80">
                <div>
                    <label for="kategori" class="mb-2 block text-sm type-control text-zinc-700 dark:text-zinc-300">Kategori</label>
                ...
                <div>
                    <label for="q" class="mb-2 block text-sm type-control text-zinc-700 dark:text-zinc-300">Cari produk</label>
            ...
            <nav class="mt-8 flex flex-wrap gap-2 pb-16 pr-20 sm:pb-0 sm:pr-0" aria-label="Filter kategori produk">
                @foreach ($categories as $category)
                    <a href="{{ route('public.products', ['kategori' => $category->slug]) }}" @if ($selectedCategory?->id === $category->id) aria-current="page" @endif class="public-filter-chip {{ $selectedCategory?->id === $category->id ? 'bg-gold-500 text-zinc-950 shadow-[0_8px_20px_rgba(254,172,24,0.18)]' : 'bg-white text-zinc-700 hover:text-zinc-950 dark:bg-white/10 dark:text-zinc-300 dark:hover:text-gold-400' }}">
                        {{ $category->name }}
                    </a>
                @endforeach
            </nav>

            <p id="products-filter-status" class="mt-5 break-words text-sm type-control text-zinc-600 dark:text-zinc-400" role="status">
            ...
            <div class="mt-10 grid items-stretch gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                ...
                @empty
                    <div class="public-card sm:col-span-2 lg:col-span-3 xl:col-span-4">
                        <h2 class="text-xl type-title text-zinc-950 dark:text-zinc-100">Produk tidak ditemukan.</h2>
