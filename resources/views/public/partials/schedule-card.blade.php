...
<article class="group public-card public-card-hover public-motion-card public-motion-reveal flex h-full flex-col" data-motion="reveal card" data-motion-delay="{{ $motionDelay }}">
    <div class="public-media-frame -mx-2 -mt-2 mb-5 aspect-[4/3]">
        ...
        <div class="absolute bottom-4 left-4 right-4 flex min-w-0 flex-wrap items-end justify-between gap-3">
            <span class="max-w-full break-words rounded-full bg-gold-500 px-3 py-1 text-xs type-control uppercase tracking-[0.12em] text-zinc-950">{{ $dayLabel }}</span>
            <span class="break-words rounded-full bg-zinc-950/80 px-3 py-1 text-xs type-control text-zinc-100 ring-1 ring-white/10">{{ substr((string) $schedule->start_time, 0, 5) }}</span>
        </div>
    </div>
    <div class="flex flex-wrap items-center gap-2">
        <span class="max-w-full break-words rounded-full bg-gold-500 px-3 py-1 text-xs type-control uppercase tracking-[0.12em] text-zinc-950">
            {{ $dayLabel }}
        </span>
        <span class="max-w-full break-words rounded-full bg-zinc-100 px-3 py-1 text-xs type-control text-zinc-600 dark:bg-white/10 dark:text-zinc-300">
            {{ $class ? \Illuminate\Support\Str::headline($class->class_type) : 'Kelas' }}
        </span>
    </div>
    <h3 class="mt-5 break-words text-xl type-title text-zinc-950 dark:text-zinc-100">{{ $class?->name ?? 'Kelas Platinum' }}</h3>
    <dl class="mt-5 space-y-3 text-sm text-zinc-600 dark:text-zinc-400">
        <div class="flex min-w-0 justify-between gap-4">
            <dt class="shrink-0 type-control text-zinc-500 dark:text-zinc-500">Waktu</dt>
            <dd class="min-w-0 break-words text-right type-control text-zinc-900 dark:text-zinc-100">{{ $timeLabel }}</dd>
        </div>
        <div class="flex min-w-0 justify-between gap-4">
            <dt class="shrink-0 type-control text-zinc-500 dark:text-zinc-500">{{ $staffRoleLabel }}</dt>
            <dd class="min-w-0 break-words text-right type-control text-zinc-900 dark:text-zinc-100">{{ $staffDisplayName !== '-' ? $staffDisplayName : 'Tim Platinum' }}</dd>
        </div>
        <div class="flex min-w-0 justify-between gap-4">
            <dt class="shrink-0 type-control text-zinc-500 dark:text-zinc-500">Kuota</dt>
            <dd class="min-w-0 break-words text-right type-control text-zinc-900 dark:text-zinc-100">{{ $schedule->capacity ?? $class?->capacity }} peserta</dd>
        </div>
        <div class="flex min-w-0 justify-between gap-4">
            <dt class="shrink-0 type-control text-zinc-500 dark:text-zinc-500">Harga</dt>
            <dd class="min-w-0 break-words text-right type-control text-zinc-700 dark:text-gold-400">
