...
<article class="group public-card public-card-hover public-motion-card public-motion-reveal flex h-full flex-col" data-motion="reveal card" data-motion-delay="{{ $motionDelay }}">
    ...
    <div class="flex flex-wrap items-center gap-2">
        <span class="max-w-full break-words rounded-full bg-zinc-100 px-3 py-1 text-xs font-bold uppercase tracking-[0.16em] text-zinc-600 dark:bg-white/10 dark:text-zinc-300">
            {{ \Illuminate\Support\Str::headline($package->package_kind) }}
        </span>
        @if ($package->promo_price)
            <span class="rounded-full bg-gold-500 px-3 py-1 text-xs font-black uppercase tracking-[0.16em] text-zinc-950">Promo</span>
        @endif
        @if ($bonusLabel)
            <span class="rounded-full bg-emerald-500/15 px-3 py-1 text-xs font-black uppercase tracking-[0.16em] text-emerald-700 dark:text-emerald-300">{{ $bonusLabel }}</span>
        @endif
    </div>
    <h3 class="mt-5 break-words text-xl font-black text-zinc-950 dark:text-white">{{ $package->name }}</h3>
    ...
    <div class="mt-6 flex flex-wrap items-end gap-x-2 gap-y-1">
        <p class="break-words text-3xl font-black text-zinc-950 dark:text-white">@include('public.partials.price', ['amount' => $package->promo_price ?? $package->price])</p>
        @if ($durationLabel)
            <p class="break-words pb-1 text-xs font-bold uppercase tracking-[0.16em] text-zinc-500">/{{ $durationLabel }}</p>
        @elseif ($package->session_count)
            <p class="break-words pb-1 text-xs font-bold uppercase tracking-[0.16em] text-zinc-500">/{{ $package->session_count }} sesi</p>
        @endif
    </div>
    @if ($package->promo_price)
        <p class="mt-1 break-words text-sm font-semibold text-zinc-500 line-through">@include('public.partials.price', ['amount' => $package->price])</p>
    @endif
