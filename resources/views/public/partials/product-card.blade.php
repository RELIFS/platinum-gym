    ...
    <div class="public-media-frame public-product-media aspect-[4/3] bg-zinc-100 dark:bg-zinc-900">
        ...
        @else
            <div class="flex h-full items-center justify-center bg-gradient-to-br from-zinc-950 via-zinc-900 to-gold-600/30">
                <span class="text-5xl type-emphasis text-gold-500">{{ \Illuminate\Support\Str::of($product->name)->substr(0, 1)->upper() }}</span>
            </div>
        @endif
        <span class="absolute right-3 top-3 rounded-full bg-zinc-950/85 px-3 py-1 text-xs type-control text-white shadow-lg ring-1 ring-white/15 dark:bg-gold-500 dark:text-zinc-950">Stok: {{ number_format($product->stock, 0, ',', '.') }}</span>
    </div>
    <div class="mt-5 flex min-w-0 flex-1 flex-col">
        <p class="break-words text-xs type-control uppercase tracking-[0.12em] text-zinc-600 dark:text-gold-400">{{ $product->category?->name ?? 'Produk' }}</p>
        <h3 class="public-product-title mt-2 break-words text-lg type-title leading-snug text-zinc-950 dark:text-zinc-100">{{ $product->name }}</h3>
        <p class="mt-3 break-words text-2xl type-emphasis leading-none text-zinc-950 dark:text-zinc-100">@include('public.partials.price', ['amount' => $product->price])</p>
