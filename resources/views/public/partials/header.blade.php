@php
    ...
    $whatsappNavUrl = $whatsappUrl.(str_contains($whatsappUrl, '?') ? '&' : '?').http_build_query([
        'text' => 'Halo Platinum Gym Padang, saya ingin tanya layanan dan jadwal.',
    ]);
    $navItems = [
...
<header class="sticky top-0 z-50 border-b border-zinc-200/80 bg-zinc-50/90 backdrop-blur-xl dark:border-white/10 dark:bg-zinc-950/90" x-data="{ open: false }">
    <div class="public-container">
        <div class="flex min-h-20 items-center justify-between gap-4">
            ...
            <nav class="hidden items-center gap-1 lg:flex" aria-label="Navigasi utama">
                @foreach ($navItems as $item)
                    ...
                    <a href="{{ route($item['route']) }}" @if ($isActive) aria-current="page" @endif class="inline-flex min-h-11 touch-manipulation items-center rounded-full px-3 py-2 text-sm font-bold transition focus:outline-none focus-visible:ring-2 focus-visible:ring-gold-500/40 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-50 dark:focus-visible:ring-offset-zinc-950 {{ $isActive ? 'bg-gold-500 text-zinc-950' : 'text-zinc-600 hover:bg-zinc-200/70 hover:text-zinc-950 dark:text-zinc-300 dark:hover:bg-white/10 dark:hover:text-white' }}">
                        {{ $item['label'] }}
                    </a>
                @endforeach
            </nav>
            ...
            <div class="flex items-center gap-2 lg:hidden">
                <x-theme-toggle />
                <button type="button" class="inline-flex h-11 w-11 touch-manipulation items-center justify-center rounded-full bg-zinc-950 text-white shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-gold-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-50 dark:bg-gold-500 dark:text-zinc-950 dark:focus-visible:ring-offset-zinc-950" x-on:click="open = !open" x-bind:aria-expanded="open.toString()" aria-controls="mobile-navigation" x-bind:aria-label="open ? 'Tutup navigasi' : 'Buka navigasi'">
        ...
        <div id="mobile-navigation" class="max-h-[calc(100dvh-6rem)] overflow-y-auto overscroll-contain pb-5 lg:hidden" x-cloak x-show="open" x-transition>
            <nav class="grid gap-2 rounded-2xl border border-zinc-200 bg-white p-3 shadow-xl dark:border-white/10 dark:bg-zinc-900" aria-label="Navigasi mobile">
                @foreach ($navItems as $item)
                    @php($isActive = request()->routeIs($item['route']))
                ...
                <div class="grid gap-2 pt-2 sm:grid-cols-2">
                    ...
                    @endauth
                    <a href="{{ $whatsappNavUrl }}" target="_blank" rel="noopener noreferrer" class="public-button-secondary sm:col-span-2">Tanya via WhatsApp</a>
                </div>
            </nav>
        </div>
    </div>
