<x-guest-layout>
    <div class="w-full">
        <a href="{{ route('register') }}" class="mb-8 inline-flex min-h-11 items-center gap-2 rounded-lg px-1 py-2 text-sm type-control text-zinc-500 transition hover:text-gold-text dark:text-zinc-400">
        ...
        <div class="mb-6">
            <div class="mb-5 inline-flex h-14 w-14 items-center justify-center rounded-2xl border border-gold-600/30 bg-gold-500/10 text-gold-text shadow-sm shadow-gold-500/10 dark:border-gold-400/25">
            ...
            <h2 class="mb-3 text-3xl type-title leading-tight text-zinc-950 dark:text-zinc-100">
                Verifikasi <span class="text-gold-display">Email</span>
            </h2>
            <p class="auth-panel-copy">
                Masukkan kode 6 digit yang dikirim ke <span class="type-control text-zinc-800 dark:text-zinc-100">{{ $maskedEmail }}</span>. Kode berlaku 10 menit.
            </p>
        </div>

        @if (session('status') == 'verification-code-sent')
            <div class="mb-5 flex gap-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-sm type-compact text-emerald-700 dark:text-emerald-300" role="status">
                ...
                <div>
                    <p class="type-control">Kode baru sudah dikirim.</p>
        ...
        <div class="rounded-2xl border border-zinc-200 bg-white/85 p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/70">
            <form method="POST" action="{{ route('verification.code.verify') }}" class="mb-6">
                ...
                <input
                    ...
                    value="{{ old('code') }}"
                    class="auth-input mt-2 text-center text-2xl type-control tracking-[0.28em] sm:text-3xl"
                    placeholder="000000"
                    aria-describedby="verification-code-help{{ $errors->has('code') ? ' verification-code-error' : '' }}"
                    @error('code') aria-invalid="true" @enderror
                    required
                >
                <p id="verification-code-help" class="mt-3 text-xs type-control leading-5 text-zinc-500 dark:text-zinc-400">Jangan bagikan kode ini kepada siapa pun, termasuk pihak yang mengaku dari Platinum Gym.</p>
                @error('code')
            ...
            <div class="rounded-xl border border-dashed border-zinc-300 bg-zinc-50/80 p-4 dark:border-zinc-700 dark:bg-zinc-950/45">
                <p class="text-sm type-control leading-relaxed text-zinc-600 dark:text-zinc-300">
                    Belum menerima kode? Cek Inbox, Spam, atau Promosi, lalu kirim ulang jika masih belum ada.
                </p>
                <p class="mt-2 text-xs type-compact leading-5 text-zinc-500 dark:text-zinc-400">
