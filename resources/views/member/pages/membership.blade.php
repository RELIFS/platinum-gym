    ...
    <div class="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1.18fr)_minmax(0,0.82fr)]">
                ...
                <div class="member-horizontal-rail -mx-1 mt-4 flex snap-x snap-mandatory gap-3 overflow-x-auto px-1 pb-3" aria-label="Daftar membership aktif" tabindex="0">
                    @foreach ($activeMemberships as $membership)
                        <article class="min-w-0 shrink-0 basis-[min(100%,20rem)] snap-start rounded-lg border border-emerald-500/20 bg-white/75 p-4 dark:border-emerald-400/15 dark:bg-zinc-950/45 sm:basis-80 xl:basis-[21rem]">
                            <h4 class="break-words text-xl type-title text-zinc-950 dark:text-zinc-100">{{ $membership->package?->name ?? $membership->code }}</h4>
                            <p class="mt-2 member-copy">{{ $membership->validityLabel() }}.</p>
                            <p class="mt-4 text-2xl type-emphasis text-zinc-950 dark:text-gold-400">Rp {{ number_format((float) $membership->price, 0, ',', '.') }}</p>
                        </article>
                    @endforeach
                </div>
            </div>
        @else
            <div class="member-soft-panel min-w-0">
                <h4 class="type-title text-zinc-950 dark:text-zinc-100">Belum ada membership aktif</h4>
            ...
            @forelse ($activePackageSessions as $session)
                <article class="member-soft-panel min-w-0">
                    <div class="flex min-w-0 flex-wrap items-start justify-between gap-2">
                        <p class="min-w-0 break-words type-control leading-snug text-zinc-950 dark:text-zinc-100">{{ $session->package?->name ?? $session->code }}</p>
                        <span class="member-status-pill member-status-info shrink-0">Sesi aktif</span>
                    </div>
                    <p class="mt-3 inline-flex min-h-8 items-center rounded-full bg-emerald-500/10 px-3 text-sm type-control text-emerald-700 dark:text-emerald-300">{{ $session->remaining_sessions }} dari {{ $session->total_sessions }} sesi tersisa</p>
                    @if ($session->trainer)
                        <div class="mt-3 min-w-0 rounded-md border border-zinc-200 bg-white/70 px-3 py-2 dark:border-white/10 dark:bg-white/[0.03]">
                            <p class="text-xs type-control text-zinc-500 dark:text-zinc-400">Trainer</p>
                            <p class="mt-1 break-words text-sm type-control text-zinc-700 dark:text-zinc-200">{{ $session->trainer->name }}</p>
                        </div>
                    @endif
                </article>
            @empty
                <article class="member-soft-panel min-w-0">
                    <p class="type-control text-zinc-950 dark:text-zinc-100">Belum ada paket sesi aktif</p>
            ...
            @foreach ($packageGroups as $group)
                <section class="min-w-0" aria-labelledby="package-group-{{ $group['key'] }}">
                    <div class="mb-4 flex min-w-0 flex-wrap items-center justify-between gap-3 border-b border-zinc-200 pb-3 dark:border-white/10">
                        <h4 id="package-group-{{ $group['key'] }}" class="break-words text-lg type-title text-zinc-950 dark:text-zinc-100">{{ $group['title'] }}</h4>
