<section class="member-card mt-6">
    ...
    <div class="mt-5 space-y-3 md:hidden">
        @forelse ($payments as $payment)
            ...
            <article class="rounded-lg border border-zinc-200 bg-white p-4 dark:border-white/10 dark:bg-zinc-950/45">
                ...
                <div class="mt-4 flex flex-col gap-2">
                    ...
                    @if (in_array($payment->status, ['waiting_payment', 'pending', 'unpaid'], true) && filled($payment->midtrans_redirect_url))
        ...
        @empty
            <div class="member-soft-panel">Belum ada transaksi. Riwayat pembayaran akan muncul setelah checkout membership, paket sesi, atau kelas.</div>
        @endforelse
    </div>
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-white/10">
                @forelse ($payments as $payment)
                    ...
                    <tr class="bg-white dark:bg-zinc-950/35">
                        ...
                        <td class="px-5 py-4"><span class="member-status-pill {{ $paymentStatusClass }}">{{ $paymentStatusLabel }}</span></td>
                        <td class="px-5 py-4">
                ...
                @empty
                    <tr><td colspan="6" class="px-5 py-8 text-center font-semibold text-zinc-500">Belum ada transaksi. Riwayat pembayaran akan muncul setelah checkout membership, paket sesi, atau kelas.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
