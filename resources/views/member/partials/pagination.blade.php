...
@if ($hasPaginator)
    <div class="mt-5 flex flex-col gap-3 border-t border-zinc-200 pt-4 dark:border-white/10 sm:flex-row sm:items-center sm:justify-between">
        <p class="text-sm type-control text-zinc-500 dark:text-zinc-400">{{ $countText }}</p>
            ...
            <nav class="flex flex-wrap gap-2" aria-label="Navigasi halaman {{ $label }}">
                ...
                @foreach (range($startPage, $endPage) as $pageNumber)
                    @if ($pageNumber === $currentPage)
                        <span class="grid min-h-11 min-w-11 place-items-center rounded-lg bg-gold-500 px-3 text-sm type-control text-zinc-950" aria-current="page">{{ $pageNumber }}</span>
                    @else
                        <a href="{{ $paginator->url($pageNumber) }}" class="grid min-h-11 min-w-11 place-items-center rounded-lg border border-zinc-200 bg-white px-3 text-sm type-control text-zinc-700 transition hover:border-gold-600/60 hover:bg-zinc-50 hover:text-gold-text focus:outline-none focus-visible:ring-2 focus-visible:ring-gold-700/40 dark:border-white/10 dark:bg-zinc-950/45 dark:text-zinc-200 dark:hover:bg-white/[0.07] dark:focus-visible:ring-gold-400/40" aria-label="Halaman {{ $pageNumber }}">{{ $pageNumber }}</a>
                    @endif
                @endforeach
