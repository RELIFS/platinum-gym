...
<div data-chatbot-root data-chatbot-config="platinumGymMemberChatbotConfig" data-chatbot-variant="member">
    <div data-chatbot-overlay hidden class="fixed inset-0 z-[35] bg-zinc-950/60 backdrop-blur-sm sm:hidden" aria-hidden="true"></div>

    <div class="fixed bottom-3 right-3 z-[45] flex w-fit max-w-[calc(100vw-1.5rem)] flex-col items-end sm:bottom-6 sm:right-6" style="padding-bottom: env(safe-area-inset-bottom);">
        <section data-chatbot-panel hidden tabindex="-1" role="dialog" aria-modal="true" aria-hidden="true" aria-labelledby="member-chatbot-title" class="mx-auto flex h-[min(620px,calc(100dvh-1.5rem))] w-[calc(100vw-1.5rem)] flex-col overflow-hidden overscroll-contain rounded-2xl border border-zinc-800 bg-zinc-950 shadow-2xl focus:outline-none focus-visible:ring-2 focus-visible:ring-gold-500/50 sm:h-[min(560px,calc(100dvh-6rem))] sm:w-96">
            <header class="flex shrink-0 items-center justify-between gap-3 bg-gold-500 px-4 py-3 text-zinc-950">
                <div class="flex min-w-0 items-center gap-3">
                    <span class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-zinc-950/10" aria-hidden="true">
                        @include('member.partials.icon', ['name' => 'spark', 'class' => 'h-5 w-5'])
                    </span>
                    <div class="min-w-0">
                        <h2 id="member-chatbot-title" class="truncate text-sm font-black">Chatbot Member</h2>
                        <p class="flex items-center gap-1.5 text-xs font-semibold text-zinc-950/75">
                            ...
                            Bantuan cepat portal
                        </p>
                    </div>
                </div>
                <button type="button" data-chatbot-close class="inline-flex h-11 w-11 shrink-0 touch-manipulation items-center justify-center rounded-full text-zinc-950 transition hover:bg-zinc-950/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-zinc-950/30" aria-label="Tutup chatbot member">
            ...
            <div data-chatbot-messages class="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain px-4 py-4" aria-live="polite" aria-relevant="additions text"></div>
            ...
            <div class="shrink-0 border-t border-zinc-800 px-3 py-3">
                ...
                <div class="flex gap-2">
                    <input type="text" data-chatbot-input class="public-input h-11 min-w-0 flex-1 border-zinc-700 bg-zinc-900 text-base text-zinc-100 placeholder:text-zinc-500" placeholder="Ketik pertanyaan..." aria-label="Ketik pertanyaan chatbot member">
                    <button type="button" data-chatbot-send class="inline-flex h-11 w-11 shrink-0 touch-manipulation items-center justify-center rounded-full bg-gold-500 text-zinc-950 transition hover:bg-gold-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-gold-500/60 disabled:cursor-not-allowed disabled:opacity-50" aria-label="Kirim pesan chatbot member" disabled>
        ...
        <button type="button" data-chatbot-trigger class="group inline-flex h-12 w-12 touch-manipulation items-center justify-center gap-3 rounded-full bg-gold-500 p-0 text-zinc-950 shadow-[0_18px_50px_rgba(254,172,24,0.34)] ring-1 ring-zinc-950/10 transition hover:bg-gold-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-gold-500 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-50 dark:focus-visible:ring-offset-zinc-950 sm:h-auto sm:min-h-14 sm:w-auto sm:px-4 sm:py-3" aria-label="Buka chatbot member Platinum Gym">
            ...
            <span class="hidden leading-tight 2xl:block">
                ...
                <span class="block text-sm font-black">Bantuan Member</span>
            </span>
        </button>
    </div>
</div>
