        ...
        <div x-data="{ ownerMenuOpen: false, lastFocusedEl: null, openOwnerMenu() { this.lastFocusedEl = document.activeElement; this.ownerMenuOpen = true; this.$nextTick(() => this.$refs.ownerMobilePanel?.querySelector('a[href], button:not([disabled])')?.focus()); }, closeOwnerMenu() { this.ownerMenuOpen = false; this.$nextTick(() => this.lastFocusedEl?.focus?.()); }, trapOwnerMenu(event) { if (! this.ownerMenuOpen || ! this.$refs.ownerMobilePanel) return; const focusable = Array.from(this.$refs.ownerMobilePanel.querySelectorAll('a[href], button:not([disabled]), [tabindex]:not([tabindex=&quot;-1&quot;])')).filter((el) => el.offsetParent !== null); if (! focusable.length) return; const first = focusable[0]; const last = focusable[focusable.length - 1]; if (event.shiftKey && document.activeElement === first) { event.preventDefault(); last.focus(); } else if (! event.shiftKey && document.activeElement === last) { event.preventDefault(); first.focus(); } } }" x-on:keydown.escape.window="ownerMenuOpen && closeOwnerMenu()" x-on:keydown.tab="trapOwnerMenu($event)" class="min-h-dvh bg-[#f8fafc] dark:bg-zinc-950">
            <div class="fixed inset-y-0 left-0 z-40 hidden w-72 border-r border-zinc-200 bg-white/95 shadow-[14px_0_48px_rgba(15,23,42,0.055)] backdrop-blur-xl print:hidden dark:border-white/10 dark:bg-zinc-950/95 lg:flex lg:flex-col">
                <div class="flex min-h-[4.5rem] items-center border-b border-zinc-200 px-5 dark:border-white/10">
                    <a href="{{ route('owner.dashboard') }}" class="inline-flex min-h-11 min-w-0 touch-manipulation items-center rounded-md focus:outline-none focus-visible:ring-2 focus-visible:ring-gold-500/40 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:focus-visible:ring-offset-zinc-950" aria-label="Dashboard owner Platinum Gym">
            ...
            <aside id="owner-mobile-navigation" x-ref="ownerMobilePanel" x-cloak x-bind:class="ownerMenuOpen ? 'flex' : 'hidden'" class="fixed inset-y-0 left-0 z-[55] w-[86%] max-w-[20rem] flex-col border-r border-zinc-200 bg-white text-zinc-950 shadow-2xl print:hidden dark:border-white/10 dark:bg-zinc-950 dark:text-white lg:hidden" role="dialog" aria-modal="true" aria-label="Menu owner mobile">
            ...
            <div class="lg:pl-72">
                <header class="sticky top-0 z-30 border-b border-zinc-200 bg-[#f8fafc]/90 backdrop-blur-xl print:hidden dark:border-white/10 dark:bg-zinc-950/90">
