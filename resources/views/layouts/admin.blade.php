...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="scroll-smooth">
    ...
    <body class="bg-[#f8fafc] font-sans text-zinc-950 antialiased selection:bg-gold-500 selection:text-zinc-950 dark:bg-zinc-950 dark:text-zinc-100">
        ...
        <div x-data="{ adminMenuOpen: false, lastFocusedEl: null, syncAdminMenuState() { window.platinumGymUi?.setMobileMenuOpen('admin-navigation', this.adminMenuOpen); }, openAdminMenu() { this.lastFocusedEl = document.activeElement; this.adminMenuOpen = true; this.syncAdminMenuState(); this.$nextTick(() => this.$refs.adminMobilePanel?.querySelector('a[href], button:not([disabled])')?.focus()); }, closeAdminMenu() { this.adminMenuOpen = false; this.syncAdminMenuState(); this.$nextTick(() => this.lastFocusedEl?.focus?.()); }, trapAdminMenu(event) { if (! this.adminMenuOpen || ! this.$refs.adminMobilePanel) return; const focusable = Array.from(this.$refs.adminMobilePanel.querySelectorAll('a[href], button:not([disabled]), [tabindex]:not([tabindex=&quot;-1&quot;])')).filter((el) => el.offsetParent !== null); if (! focusable.length) return; const first = focusable[0]; const last = focusable[focusable.length - 1]; if (event.shiftKey && document.activeElement === first) { event.preventDefault(); last.focus(); } else if (! event.shiftKey && document.activeElement === last) { event.preventDefault(); first.focus(); } } }" x-init="syncAdminMenuState()" x-on:keydown.escape.window="adminMenuOpen && closeAdminMenu()" x-on:keydown.tab="trapAdminMenu($event)" x-on:resize.window="if (window.innerWidth >= 1024 && adminMenuOpen) closeAdminMenu()" class="min-h-dvh bg-[#f8fafc] dark:bg-zinc-950">
            <div class="fixed inset-y-0 left-0 z-40 hidden w-72 border-r border-zinc-200 bg-white/95 shadow-[14px_0_48px_rgba(15,23,42,0.055)] backdrop-blur-xl dark:border-white/10 dark:bg-zinc-950/95 lg:flex lg:flex-col">
                <div class="flex min-h-[4.5rem] items-center border-b border-zinc-200 px-5 dark:border-white/10">
                    <a href="{{ route('admin.dashboard') }}" class="inline-flex min-w-0 items-center rounded-md focus:outline-none focus-visible:ring-2 focus-visible:ring-gold-700/40 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:focus-visible:ring-gold-400/40 dark:focus-visible:ring-offset-zinc-950" aria-label="Dashboard admin Platinum Gym">
            ...
            <aside id="admin-mobile-navigation" x-ref="adminMobilePanel" x-cloak x-bind:class="adminMenuOpen ? 'flex' : 'hidden'" class="fixed inset-y-0 left-0 z-[55] w-[86%] max-w-[20rem] flex-col border-r border-zinc-200 bg-white text-zinc-950 shadow-2xl dark:border-white/10 dark:bg-zinc-950 dark:text-zinc-100 lg:hidden" role="dialog" aria-modal="true" aria-label="Menu admin mobile">
                ...
                <div class="border-t border-zinc-200 p-4 dark:border-white/10">
                    <div class="mb-3 flex items-center gap-3 rounded-lg border border-zinc-200 bg-zinc-50/90 p-3 dark:border-white/10 dark:bg-white/[0.045]" aria-label="Identitas admin">
                        <span class="grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-full bg-gold-500 text-sm type-control text-zinc-950" aria-hidden="true">
                            @if ($adminAvatarUrl)
                        ...
                        <div class="min-w-0">
                            <p class="truncate text-sm type-control text-zinc-950 dark:text-zinc-100">{{ $adminName }}</p>
                            <p class="mt-0.5 truncate text-[0.7rem] type-control uppercase tracking-[0.1em] text-gold-text">{{ $adminRoleLabel }}</p>
                        </div>
                    </div>
            ...
            <div class="lg:pl-72">
                <header data-auto-hide-topbar data-auto-hide-scope="below-lg" class="sticky top-0 z-30 border-b border-zinc-200 bg-[#f8fafc]/90 backdrop-blur-xl dark:border-white/10 dark:bg-zinc-950/90">
                    <div class="mx-auto flex min-h-16 w-full max-w-7xl items-center justify-between gap-3 px-4 sm:px-6 lg:min-h-20 lg:px-8">
                        ...
                        <div class="flex items-center gap-2 sm:gap-3">
                            <a href="{{ route('admin.notifications') }}" class="relative inline-flex h-11 min-w-11 items-center justify-center rounded-full border border-zinc-200 bg-white px-2 text-zinc-700 shadow-sm transition hover:border-gold-600/60 hover:text-gold-text focus:outline-none focus-visible:ring-2 focus-visible:ring-gold-700/40 dark:border-white/10 dark:bg-white/[0.04] dark:text-zinc-300 dark:focus-visible:ring-gold-400/40" aria-label="{{ $pendingApprovalAria }}">
                                @include('admin.partials.icon', ['name' => 'bell', 'class' => 'h-4 w-4'])
                                @if ($pendingApprovalCount > 0)
                                    <span class="absolute -right-1 -top-1 inline-flex min-h-5 min-w-5 items-center justify-center rounded-full bg-gold-500 px-1 text-[0.65rem] type-control leading-none text-zinc-950 ring-2 ring-white dark:ring-zinc-950" aria-hidden="true">{{ $pendingApprovalBadgeText }}</span>
                                    <span class="sr-only">{{ $pendingApprovalCount }} persetujuan menunggu review</span>
                                @endif
                            </a>
                            <a href="{{ route('admin.payments') }}" class="inline-flex items-center gap-1.5 rounded-full border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs type-control text-amber-700 transition hover:border-amber-500/50 hover:bg-amber-500/15 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/40 dark:text-amber-300" aria-label="{{ $pendingBadgeAria }}">
