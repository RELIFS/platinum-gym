    ...
    <div class="grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.72fr)] lg:items-start">
        <div class="flex min-w-0 flex-col gap-5 sm:flex-row sm:items-center">
            ...
            <div class="min-w-0">
                ...
                <h2 class="mt-2 break-words text-2xl font-black leading-tight text-zinc-950 dark:text-white">{{ $adminName }}</h2>
        ...
        >
            ...
            <div class="mt-3 flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center">
                <label for="admin_avatar" class="admin-button-secondary cursor-pointer peer-focus-visible:ring-2 peer-focus-visible:ring-gold-500/40 peer-focus-visible:ring-offset-2 peer-focus-visible:ring-offset-white dark:peer-focus-visible:ring-offset-zinc-950">
                    @include('admin.partials.icon', ['name' => 'user', 'class' => 'h-4 w-4'])
                    Pilih Foto
                </label>
                <span class="min-w-0 break-words text-sm font-bold text-zinc-600 dark:text-zinc-300" x-text="fileName || 'Belum ada foto dipilih'">Belum ada foto dipilih</span>
            </div>
            ...
            <p class="mt-2 text-xs font-black uppercase tracking-[0.12em] text-gold-700 dark:text-gold-400" x-show="avatarPreviewActive" x-cloak>Preview tampil di avatar utama, belum disimpan</p>
            @error('avatar')
