    ...
    <div class="flex min-w-0 flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div class="min-w-0">
            <div class="flex min-w-0 flex-wrap items-center gap-2">
                ...
                <span class="inline-flex min-h-7 items-center rounded-full border border-gold-500/25 bg-gold-500/10 px-3 py-1 text-xs type-control text-gold-text">
                    {{ $period }}
                </span>
            </div>
            <h2 class="mt-2 text-xl type-title text-zinc-950 dark:text-zinc-100">{{ $trend['title'] ?? 'Tren aktivitas' }}</h2>
            ...
            @foreach ($series as $item)
                ...
                <div class="min-w-0 rounded-lg border border-zinc-200 bg-white px-3 py-2 shadow-sm dark:border-white/10 dark:bg-white/[0.045]">
                    <div class="flex min-w-0 items-center gap-2">
                        ...
                        <span class="truncate text-xs type-control text-zinc-600 dark:text-zinc-300">{{ $item['name'] ?? '-' }}</span>
                    </div>
                    <p class="mt-1 text-lg type-control leading-tight text-zinc-950 dark:text-zinc-100">{{ $total }}</p>
                </div>
            @endforeach
        </div>
    </div>
