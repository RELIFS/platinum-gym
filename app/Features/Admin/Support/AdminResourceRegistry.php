...
class AdminResourceRegistry
{
    ...
    public function definitions(): array
    {
        return [
            ...
            'class-schedules' => $this->resource('Jadwal Kelas', ClassSchedule::class, 'manage_classes', 'admin.classes', [
    ...
    public function rules(string $resource, ?Model $model = null): array
    {
        return match ($resource) {
            ...
            'products' => $this->slugRules('products', $model) + ['category_id' => ['nullable', 'exists:product_categories,id'], 'name' => ['required', 'string', 'max:150'], 'price' => ['required', 'numeric', 'min:0'], 'stock' => ['required', 'integer', 'min:0'], 'description' => ['nullable', 'string', 'max:2500'], 'image_file' => ['nullable', 'image', 'max:2048'], 'image_alt' => ['nullable', 'string', 'max:180'], 'is_active' => ['boolean']],
            'gallery' => ['title' => ['nullable', 'string', 'max:150'], 'caption' => ['nullable', 'string', 'max:255'], 'image_file' => ['nullable', 'image', 'max:4096'], 'image_alt' => ['nullable', 'string', 'max:180'], 'sort_order' => ['nullable', 'integer', 'min:0'], 'is_published' => ['boolean']],
            ...
            'promos' => $this->slugRules('promos', $model) + ['title' => ['required', 'string', 'max:150'], 'description' => ['nullable', 'string', 'max:2500'], 'starts_at' => ['nullable', 'date'], 'ends_at' => ['nullable', 'date', 'after_or_equal:starts_at'], 'discount_type' => ['nullable', Rule::in(['none', 'percentage', 'fixed'])], 'discount_value' => ['nullable', 'numeric', 'min:0'], 'sort_order' => ['nullable', 'integer', 'min:0'], 'is_published' => ['boolean']],
    ...
    private function resource(string $title, string $model, string $permission, string $indexRoute, array $fields, string $description): array
    {
        return ['title' => $title, 'singular' => $title, 'model' => $model, 'permission' => $permission, 'index_route' => $indexRoute, 'fields' => array_map(fn (array $field): array => $this->field(...$field), $fields), 'description' => $description];
    }

    private function field(string $name, string $label, string $type, bool $required = false, array|string|null $options = null): array
    {
        return ['name' => $name, 'label' => $label, 'type' => $type, 'required' => $required, 'options' => is_array($options) ? $options : [], 'source' => is_string($options) ? $options : null];
    }
